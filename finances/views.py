...
def index(request):
    if request.method == "GET":
        ...
        return render(
            request,
            "finances/index.html",
            {"operations": operations, "is_paginated": is_paginated, "page_obj": page, "expenses": expenses},
        )
...
def operation_view(request, operation_id: int):
    if request.method == "POST":
        ...
        return render(request, "finances/operation.html", {"operation": operation})
...
def category_list(request):
    if request.method == "GET":
        return render(request, "finances/category_list.html", {"object_list": request.user.categories.all()})
    else:
...
def account_list(request):
    if request.method == "GET":
        return render(request, "finances/account_list.html", {"object_list": request.user.accounts.all()})
    else:
...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(request, "finances/login.html", {"message": "Invalid username and/or password."})
    else:
...
def register(request):
    if request.method == "POST":
        ...
        if password != confirmation:
            return render(request, "finances/register.html", {"message": "Passwords must match."})

        try:
            user = User.objects.create_user(username, email, password)
            user.save()
        except IntegrityError:
            return render(request, "finances/register.html", {"message": "Username already taken."})
        login(request, user)
        return HttpResponseRedirect(reverse("index"))
    else:
